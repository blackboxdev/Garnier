<!-- Header -->
<?php
include('_includes/header.php');
?>



<!-- Begin Content -->
	<div id="content">
		<div class="container">
			<div class="row">
				<div class="col-md-12">
					<div class="breadcrumb"> <a href="index.php">Home</a> > <a href="#">Hair Colour</a> > Olia
					</div>
				</div>
			</div>
			<div class="row">
				<div class="col-md-12">
					<div class="tajuk-besar">
						<h1>By Type - Healthier Looking Hair</h1>
					</div>
				</div>
			</div>

<!-- Product Filter-->
                    <div class="product-bar">
                  <div class="row leb-product"> 
                         <div class="col-md-6 tajuk-filter"> 
                                    Arrange according to <br/>
                        </div>
                        <div class="col-md-2">
                            <select  class="filter-btn" name="forma" onchange="location = this.options[this.selectedIndex].value;">
                                <option value="0">Type</option>
                                <option value="healthier.php" /> Healthier Looking Hair<br />
                                <option value="amonia.php" /> Ammonia Free<br />
                                <option value="affordability.php" /> Affordability<br />
                            </select>
                        </div>
                        <div class="col-md-2" style="margin-left:30px">
                            <select  class="filter-btn" name="forma" onchange="location = this.options[this.selectedIndex].value;">
                                <option value="0">Color </option>
                                <option value="black.php" /> Black<br />
                                <option value="brown.php" /> Brown<br />
                                <option value="red.php" /> Red<br />
                            </select>
                        </div>
                                 
                    </div>
                </div>



<!-- Products Listed -->

			<div class="row">
					<div class="col-xs-4 tolak-product  1">
						<div class=" a">
            				<img src="images/haircolor/Olia1-HitamAlami.png" class='hold' />
        				</div>
				        <div class="col-xs-6 b">
				           <b>Olia 1.0 Darkest Black</b><p>The first oil powered Ammonia free hair colour

							</p><a class="bar-findout bar-align" href="HC-olia-Darkest Black.php"> &#9656; Find Out </a>
       					</div>
					</div>


					<div class="col-xs-4 tolak-product  2">
						<div class=" a">
            				<img src="images/haircolor/Olia3.0-CoklatGelap.png" class='hold' />
        				</div>
				        <div class="col-xs-6 b">
				           <b>Olia  3.0 - Soft Black</b><p>The first oil powered Ammonia free hair colour
							</p><a class="bar-findout bar-align" href="HC-olia-SoftBlack.php"> &#9656; Find Out </a>
       					</div>
					</div>
					<div class="col-xs-4 tolak-product  1">
						<div class=" a">
            				<img src="images/haircolor/Olia4.0-DarkBrown.png" class='hold' />
        				</div>
				        <div class="col-xs-6 b">
				           <b>Olia  4.0 - Dark Brown</b><p>The first oil powered Ammonia free hair colour

							</p><a class="bar-findout bar-align" href="HC-olia-DarkBrown.php"> &#9656; Find Out </a>
       					</div>
					</div>


					<div class="col-xs-4 tolak-product  1">
						<div class=" a">
            				<img src="images/haircolor/Olia4.png" class='hold' />
        				</div>
				        <div class="col-xs-6 b">
				           <b>Olia  4.15 - Iced Chocolate</b><p>The first oil powered Ammonia free hair colour

							</p><a class="bar-findout bar-align" href="HC-IcedChocolate.php"> &#9656; Find Out </a>
       					</div>
					</div>


					<div class="col-xs-4 tolak-product  1">
						<div class=" a">
            				<img src="images/haircolor/Olia4.3GoldenDarkBrown.png" class='hold' />
        				</div>
				        <div class="col-xs-6 b">
				           <b>Olia  4.3- Golden Dark Brown</b><p>The first oil powered Ammonia free hair colour

							</p><a class="bar-findout bar-align" href="HC-GoldenDarkBrown.php"> &#9656; Find Out </a>
       					</div>
					</div>

					

					<div class="col-xs-4 tolak-product  1">
						<div class=" a">
            				<img src="images/haircolor/Olia5.png" class='hold' />
        				</div>
				        <div class="col-xs-6 b">
				           <b>Olia  5.5 - Mahogany Brown</b><p>The first oil powered Ammonia free hair colour

							</p><a class="bar-findout bar-align" href="HC-MahoganyBrown.php"> &#9656; Find Out </a>
       					</div>
					</div>

					<div class="col-xs-4 tolak-product  1">
						<div class=" a">
            				<img src="images/haircolor/Olia5.25-FrostyChestnut.png" class='hold' />
        				</div>
				        <div class="col-xs-6 b">
				           <b>Olia 5.25 - Frosty Chestnut</b><p>The first oil powered Ammonia free hair colour

							</p><a class="bar-findout bar-align" href="HC-olia-FrostyChestnut.php"> &#9656; Find Out </a>
       					</div>
					</div>

			

					<div class="col-xs-4 tolak-product  1">
						<div class=" a">
            				<img src="images/haircolor/Olia6.png" class='hold' />
        				</div>
				        <div class="col-xs-6 b">
				           <b>Olia 6.0 - Light Brown</b><p>The first oil powered Ammonia free hair colour

							</p><a class="bar-findout bar-align" href="HC-olia-LightBrown.php"> &#9656; Find Out </a>
       					</div>
					</div>

					<div class="col-xs-4 tolak-product  1">
						<div class=" a">
            				<img src="images/haircolor/Olia66.png" class='hold' />
        				</div>
				        <div class="col-xs-6 b">
				           <b>Olia 6.60 - Intense Red</b><p>The first oil powered Ammonia free hair colour

							</p><a class="bar-findout bar-align" href="HC-olia-66.php"> &#9656; Find Out </a>
       					</div>
					</div>
			</div>

			</div>
		</div>

<!-- End OF Begin Content -->







<?php
include('_includes/footer.php');
?>



